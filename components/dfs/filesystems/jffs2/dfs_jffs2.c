...
static int dfs_jffs2_open(struct dfs_fd* file)
{
	...
	mode = 0;
	if (oflag & O_RDONLY) mode |= JFFS2_O_RDONLY;
...
//	if (oflag & O_APPEND) mode |= JFFS2_O_APPEND;
	rt_mutex_take(&jffs2_lock, RT_WAITING_FOREVER);
...
static const struct dfs_file_ops dfs_jffs2_fops = 
{
	dfs_jffs2_open,
	dfs_jffs2_close,
	dfs_jffs2_ioctl,
...
static const struct dfs_filesystem_ops dfs_jffs2_ops = 
{
